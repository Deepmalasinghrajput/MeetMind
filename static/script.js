/* AI Meeting Assistant — script.js
...
function switchTab(tab) {
  state.activeTab = tab === "youtube" ? "youtube" : "file";
  document.querySelectorAll(".input-tab").forEach(t => t.classList.remove("active"));
  document.querySelectorAll(".tab-content").forEach(t => t.classList.remove("active"));
  const key = state.activeTab === "youtube" ? "Youtube" : "File";
...
async function startPipeline() {
  ...
  try {
    ...
    if (!response.ok) {
      const err = await response.json().catch(() => ({
        error: response.status >= 500
          ? "Server restarted or ran out of memory while processing. Use a short audio file (under 1–2 min) and try Upload again."
          : "Server error",
      }));
      if (response.status === 401 || err.auth_required) {
        throw new Error("Authentication required");
      }
      let msg = err.error || `HTTP ${response.status}`;
      if (/not a bot|sign in|cookies|youtube blocked/i.test(msg)) {
        msg =
          "YouTube download is blocked on this cloud server. Switch to the Upload file tab and upload a short audio/video instead.";
      }
      throw new Error(msg);
    }
    ...
    // Ensure UI never stays stuck in "processing" if an unexpected path runs
