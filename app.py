"""
AI Meeting Assistant — Flask UI wrapper for the AI Meeting Intelligence pipeline.
...
def _check_ready() -> dict:
    ...
    groq = bool(raw_key) and raw_key.lower() not in placeholder_markers and not raw_key.lower().startswith("your_")
    try:
    ...
    try:
        ...
        with track_stage("upload_to_summary", user_id=user_id):
            ...
            if not transcript or not transcript.strip():
                raise ValueError(
                    "Transcription produced empty text. Try another video or file."
                )
        ...
        msg = str(exc)
        low = msg.lower()
        if "not a bot" in low or "sign in" in low or "youtube blocked" in low or "cookies" in low:
            msg = (
                "YouTube download is blocked on this cloud server. "
                "Please use Upload file with a short audio/video instead."
            )
        elif "GROQ" in msg.upper() or "api_key" in low or "401" in msg:
            msg = (
                f"{msg}. Check GROQ_API_KEY in your .env / Render environment and restart."
            )
        elif "killed" in low or "memory" in low:
            msg = (
                "The server ran out of memory while processing. "
                "Use a shorter file and set WHISPER_MODEL=tiny on Render."
            )
...
if __name__ == "__main__":
    ...
    print(f"AI Meeting Assistant -> http://127.0.0.1:{port}")
    if host in ("0.0.0.0", "::"):
